...
const confirmEmail = async (req,res)=>{
    ...
    const propietarioBDD = await propietario.findOne({token:req.params.token})
...
const actualizarPerfil = async (req,res)=>{
    ...
    if (Object.values(req.body).includes("")) return res.status(400).json({msg:"Lo sentimos, debes llenar todos los campos"})
    const propietarioBDD = await propietario.findById(id)
    ...
    {
        const propietarioBDDMail = await propietario.findOne({email:req.body.email})
...
const actualizarPassword = async (req,res)=>{
    const propietarioBDD = await propietario.findById(req.propietarioBDD._id)
...
const recuperarPassword = async (req,res)=>{
    ...
    const propietarioBDD = await propietario.findOne({email})
...
const actualizarEmail =async (req,res)=>{
    const {email}= req.body
    if (Object.values(req.body).includes("")) return res.status(404).json({msg:"Lo sentimos, debes llenar todos los campos"})
    const verifyemailUser = propietario.findOne({email})
...
const confirmarTienda = async (req,res)=>{
    ...
    //: ACTIVIDAD 2
    const propietario = await propietario.findOne({ tokentienda });
